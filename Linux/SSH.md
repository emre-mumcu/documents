...
By default, the keys will be stored in the `~/.ssh` directory within your user’s home directory. The private key will be called `id_rsa` and the associated public key will be called `id_rsa.pub`. The private key `id_rsa` should be keet secure, while public key `id_rsa.pub` will be shared. 
...
## Setp 2. Copying an SSH Public Key to Your Server
...
If you already have an SSH key and just need to use it to connect to a server, follow these steps:
